...
async function main(): Promise<void> {
  ...
  g = await game();
  const target = g.cats.find((c) => c.phase === 'alive');
  if (!target) {
    step('kill a cat', false, 'no live cat to shoot');
  } else {
    const killsBefore = g.kills;
    // Stand 5m south of the cat, aim at its body centre (y≈0.44).
    await page.evaluate(
      ([cx, cy, cz]) => {
        const px = cx!;
        const pz = cz! + 5;
        const dy = cy! + 0.44 - (0.9 + 0.72); // body centre vs eye height
  ...
  await page.evaluate(() => window.__cod.setPlayer(0, 0.9, 0, 0, 0));
