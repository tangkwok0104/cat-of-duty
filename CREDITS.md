...
| `public/assets/gen/audio/*.mp3` (8: per-gun shots ×4, reload, dryfire, concrete impact, projectile whoosh) | Weapon SFX | Same pipeline, 2026-08-09 | Original generated content — **4 of 8 (shot-smg, reload, impact-concrete, projectile-whoosh) came back from generation near-silent (-41 to -57 dBFS peak, confirmed by spectrogram — no real transient content); SoundBus.ts auto-detects this at decode time and falls back to its synth for those specific cues. rifle/sniper/shotgun/dryfire are good. Regenerate the 4 broken ones through a proper SFX pipeline (not this MCP session's speech-only audio tool) before relying on them.** |
